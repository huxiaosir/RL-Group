...
class ResNet50(nn.Module):
    ...
    def __init__(self, block, block_num, num_classes=34):
        super(ResNet50, self).__init__()
        self.in_channel = 64  # conv1的输出维度

        self.conv1 = nn.Conv2d(in_channels=418, out_channels=self.in_channel, kernel_size=7, stride=1, padding=3,
        ...
        self.layer2 = self._make_layer(block=block, channel=128, block_num=block_num[1],
                                       stride=1)  # H/2, W/2。downsample控制的shortcut，out_channel=128x4=512
        self.layer3 = self._make_layer(block=block, channel=256, block_num=block_num[2],
                                       stride=1)  # H/2, W/2。downsample控制的shortcut，out_channel=256x4=1024
        self.layer4 = self._make_layer(block=block, channel=512, block_num=block_num[3],
                                       stride=1)  # H/2, W/2。downsample控制的shortcut，out_channel=512x4=2048
